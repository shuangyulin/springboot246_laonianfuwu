<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="店铺编号" prop="dianpubianhao">
            <el-input v-model="ruleForm.dianpubianhao" 
                placeholder="店铺编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.dianpubianhao" label="店铺编号" prop="dianpubianhao">
              <el-input v-model="ruleForm.dianpubianhao" 
                placeholder="店铺编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.dianpuzhaopian" label="店铺照片" prop="dianpuzhaopian">
          <file-upload
          tip="点击上传店铺照片"
          action="file/upload"
          :limit="3"
          ...
          @change="dianpuzhaopianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.dianpuzhaopian" label="店铺照片" prop="dianpuzhaopian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.dianpuzhaopian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.dianpujianjie" label="店铺简介" prop="dianpujianjie">
                    <span v-html="ruleForm.dianpujianjie"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shangjiamingcheng = true;
          ...
          if(o=='dianpuzhaopian'){
            this.ruleForm.dianpuzhaopian = obj[o];
	    this.ro.dianpuzhaopian = true;
            continue;
          }
          if(o=='kaifangshijian'){
	...
	this.ruleForm.dianpujianjie = this.ruleForm.dianpujianjie.replace(reg,'../../../springboot4nsk1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `dianpuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.dianpuxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    dianpuzhaopianUploadChange(fileUrls) {
