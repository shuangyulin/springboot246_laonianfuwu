      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="活动分类" prop="huodongfenlei">
          <el-input v-model="ruleForm.huodongfenlei" 
              placeholder="活动分类" clearable  :readonly="ro.huodongfenlei"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="活动分类" prop="huodongfenlei">
              <el-input v-model="ruleForm.huodongfenlei" 
...
export default {
  ...
  methods: {
	...
	this.ruleForm.huodongneirong = this.ruleForm.huodongneirong.replace(reg,'../../../springboot4nsk1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhiyuanhuodong/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhiyuanhuodongCrossAddOrUpdateFlag = false;
						 this.parent.search();
