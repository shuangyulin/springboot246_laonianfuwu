    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="店铺名称" prop="dianpumingcheng">
          <el-select :disabled="ro.dianpumingcheng" v-model="ruleForm.dianpumingcheng" placeholder="请选择店铺名称">
            <el-option
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.shangpintupian" label="商品图片" prop="shangpintupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shangpintupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.shangpinxiangqing = this.ruleForm.shangpinxiangqing.replace(reg,'../../../springboot4nsk1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
