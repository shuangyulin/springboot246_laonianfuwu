<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="统计名称" prop="tongjimingcheng">
          <el-input v-model="ruleForm.tongjimingcheng" 
              placeholder="统计名称" clearable  :readonly="ro.tongjimingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="统计名称" prop="tongjimingcheng">
              <el-input v-model="ruleForm.tongjimingcheng" 
                placeholder="统计名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="订单数" prop="dingdanshu">
          <el-input v-model="ruleForm.dingdanshu" 
              placeholder="订单数" clearable  :readonly="ro.dingdanshu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="订单数" prop="dingdanshu">
              <el-input v-model="ruleForm.dingdanshu" 
                placeholder="订单数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.tongjishijian = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tongjibianhao = true;
          ...
          if(o=='tongjimingcheng'){
            this.ruleForm.tongjimingcheng = obj[o];
	    this.ro.tongjimingcheng = true;
            continue;
          }
          if(o=='dingdanshu'){
            this.ruleForm.dingdanshu = obj[o];
	    this.ro.dingdanshu = true;
            continue;
          }
          if(o=='tongjishijian'){
	...
	this.ruleForm.beizhu = this.ruleForm.beizhu.replace(reg,'../../../springboot4nsk1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tongjishangpin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tongjishangpinCrossAddOrUpdateFlag = false;
						 this.parent.search();
